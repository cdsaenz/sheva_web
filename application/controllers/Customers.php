...
class Customers extends CDS_Controller {
            ...
            // keyword
      			if (isset($filter["keyword"]) && ($filter["keyword"] != "")) {
      			    $v = strtoupper($filter["keyword"]);
      				  $w = $this->customers_model->add_search_param($w,"WHERE","UPPER(cust_name) LIKE '%$v%'", "");
                // new store the formvar (ex: keyword)
                $w = $this->customers_model->add_search_param($w,"FORMVAR","keyword","", $v);
            }

            // save to sid
      			$sid    = $this->customers_model->save_search($w,$this->customers_model->db_table);
            $page   = 1;
        }
        else {
...

	{
        ...
        $extra_where = [];
    		...
    		$result      = $this->customers_model->get_records($page,"customers/search/$sid/",$isid, $extra_where);
        ...
        // title
        $data["title"]   = $sid == 0 ? _("Clientes") : _("Clientes (Filtrado)") ;

        // get the view
        $this->load_view('customers/list',$data);
    ...
    public function view()
    {
        ...
        $row             = $this->customers_model->get_record($id);
        ...
        if (!$row) {
            ...
            // get the view
            $this->load_view('customers/view', $data);
        }
    }
   ...
   /**
    ...
    public function edit()
    {
        ...
        $data['hidden']  = array('edit_mode' => 'edit',
                                 'key_id'    => $id   );
        $data['title']   = $row['cust_name']. _(" - Editar Cliente");

        // get the view
        $this->load_view('customers/edit', $data);
    }
     ...
     */
...

     {
        ...
        if ($mode == 'edit') {
            ...
            $this->customers_model->update_record($parms,$id);
            // go back
            redirect("customers/view/$id","refresh");
        } else if ($mode == 'add') {
			       $id = $this->customers_model->insert_record($parms);
            // go back
            redirect("customers/view/$id","refresh");
		    }
...

    {
        ...
        $data['title']   = _("Nuevo Cliente");

        // get the content FROM this view (process with vars) into a var
        $data['row']       = $this->customers_model->get_record(0);
    ...
    public function topdf() {
        ...
        $sql = "SELECT cust_name Nombre, id Codigo
                    FROM cust_mst";

        // get the query
        $query = $this->db->query($sql);
        // QUICK OPTION: transform to table etc and output to the pdf screen
        to_pdf($query, 'Clientes', false, 'clientes.pdf');
    }
